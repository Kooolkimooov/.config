...
case "$ws" in
  ''|null) echo "Could not determine the focused workspace" >&2; exit 1 ;;
esac

# hl.dsp.* only builds a dispatcher descriptor; hl.dispatch() is what runs it
case "$1" in
  move) hyprctl eval "hl.dispatch(hl.dsp.window.move({ workspace = \"special:s$ws\" }))" >/dev/null ;;
  '')   hyprctl eval "hl.dispatch(hl.dsp.workspace.toggle_special(\"s$ws\"))" >/dev/null ;;
  *)    echo "Usage: ${0##*/} [move]" >&2; exit 1 ;;
esac
